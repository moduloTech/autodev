...
| Any GitLab read the poll acts on is unreachable (job list, MR discussions) | Same: `GitlabHelpers.answer` converts the error into `ApiUnavailableError` at the read, `PipelineMonitor#check` / `MrFixer#fix` / `recheck_infra_recovery` catch it at their boundary, the row keeps its status and is re-read next cycle, one log line says which endpoint went dark. Not *bit for bit* untouched, though: `check_stagnation_and_fix` counts the stagnation signature before it attempts the fix, so a selective outage on the issue endpoint still advances that counter and can expire the ticket on stagnation — known, open, and not what the mechanism promises. There is no return value meaning "unknown", which is what made an outage indistinguishable from good news (Autodev #62) |
