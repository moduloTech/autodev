...
| Stagnation detected (pipeline or discussions) | `abandon` → `done` + `needs_attention` (`stagnation_pipeline` / `stagnation_discussions`) + `label_done` + reassignment to the author + alert comment |
| Review limit reached (3 rounds) | `abandon` → `done` + `needs_attention` (`review_limit_reached`) + `label_done` + reassignment to the author + alert comment |
