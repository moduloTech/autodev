...
| Pipeline manual / skipped, jobs endpoint unreachable | Stay in checking_pipeline, recheck next poll (an API error must never read as "nothing failed"), and the age bound stands down for that cycle (Autodev #56) |
...
| Same, but the poll never read a pipeline status (GitLab error on the job list, Claude quota exhausted) | The bound stands down: the row stays in `checking_pipeline` for the next cycle and one log line says why (Autodev #56). Only a poll that actually read a verdict can expire a watch |
