...
| Project declares `review_skill` but the clone has no matching `.claude/skills/<name>/SKILL.md` | `ConfigError` (Autodev #74) — deliberately excluded from `review_with_skill`'s rescue, so it does **not** become a review failure. It escapes `launch_review` too (no rescue there), and the only thing that catches it is `PipelineMonitor#check`'s generic `rescue StandardError`, which logs and does nothing else: neither `review_failure_count` nor the five-strikes give-up fires, and `abandon_expired_watch` — the last statement of `poll_open_mr`, downstream of where this raises — is never reached either. The row is left in `checking_pipeline`, re-raising the identical error every poll, indefinitely: a missing or misnamed skill is a standing misconfiguration, not a transient one, so retrying cannot fix it. Surfacing this on the dashboard needs its own attention reason (`fr`/`en` keys, a distinctness check against `dispatch_infra_recheck`'s reasons) — out of scope here; this row states what the code does today |
| Skill review ran but GitLab had not computed the MR's `diff_refs` yet | `review_with_skill` answers `:inconclusive`, not a value on the true/false axis (Autodev #74): `launch_review` sends the row back to `checking_pipeline` via `review_done!` and touches neither `review_count` nor `review_failure_count`, so the next poll runs the whole review again |
