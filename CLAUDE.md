...
- **MR `locked`** → nothing is concluded (Autodev #69). GitLab's MR state machine declares exactly four states (`opened`, `closed`, `merged`, `locked`) and `locked` is the only one carrying no verdict: `MergeRequests::MergeService` wraps the whole merge in `merge_request.in_locked_state`, and the REST reference calls it "short-lived and transitional". So it is read like a `running` pipeline — the poll leaves the row in `checking_pipeline` and the next cycle decides. Read as a verdict it abandoned an MR *in the middle of being delivered*: a public comment saying it had been closed without being merged (false), the ticket handed back, `needs_attention`, no end label. The list is an allow-list, `MrStateChecker::TRANSIENT_MR_STATES`, so a state GitLab adds tomorrow stays in the abandon category. The wait is **not** unbounded: the transient branch falls through to `abandon_expired_watch` rather than returning early from `poll_open_mr`, so an MR wedged in `locked` is given up at `pipeline_watch_max_days` like any other frozen watch — and it deliberately does not raise `poll_inconclusive!`, which would stand that bound down
- **Running** → skip
