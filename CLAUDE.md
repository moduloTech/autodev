...
| Any GitLab read the poll acts on is unreachable (job list, MR discussions) | Same: `GitlabHelpers.answer` converts the error into `ApiUnavailableError` at the read, `PipelineMonitor#check` / `MrFixer#fix` / `recheck_infra_recovery` catch it at their boundary, the row is left exactly as the previous cycle left it, one log line says which endpoint went dark. There is no return value meaning "unknown", which is what made an outage indistinguishable from good news (Autodev #62) |
