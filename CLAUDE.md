...
| Project declares `review_skill` but the clone has no matching `.claude/skills/<name>/SKILL.md` | `ConfigError`, normalised to a review failure (Autodev #74): refuses to fall back to the `mr-review` binary, which would run a different process than the one the project asked for |
