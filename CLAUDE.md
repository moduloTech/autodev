...
- **MR no longer `opened`** → the pipeline is not checked at all, and the ending depends on whether anything was delivered (Autodev #66). `merged` → `mr_closed!` → `done` + `label_done`: a real delivery. Any other state — `closed`, `locked`, anything GitLab adds later — → the shared abandon point (`abandon` event, `needs_attention: mr_closed_unmerged`, `label_attention`, ticket handed back to its author, comment). The split is on "was this delivered", not on GitLab's state vocabulary: posing `label_done` on a rejected MR announced it as `Development::Awaiting Feature Review`, and leaving the row unflagged put it in `dispatch_done_unassigned`'s population, so #60's `post_completion` guard did not apply to it
- **Running** → skip
...
| MR closed without being merged (or any other non-`opened` state) | `abandon` → `done` + `needs_attention` (`mr_closed_unmerged`) + `label_attention` + reassignment to the author + comment (Autodev #66). Nothing was delivered, so it must not carry the end label, and it must not stay in `dispatch_done_unassigned`'s population |
