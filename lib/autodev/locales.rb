...
module Locales
  TEMPLATES = {
    fr: {
      ...
      mr_fix_success: ':wrench: %<tag>s : %<count>s commentaire(s) de review corrige(s) sur %<mr_url>s (round %<round>s)',
      ...
      pipeline_no_failed_jobs: ":warning: %<tag>s : le pipeline de %<mr_url>s a echoue mais aucun job en echec n'a ete trouve. Intervention manuelle requise.",
      pipeline_infra_pretriage: ":warning: %<tag>s : le pipeline de %<mr_url>s echoue pour une raison d'infrastructure (pre-triage). Intervention manuelle requise.\n\n> %<explanation>s",
      pipeline_eval_failed: ":warning: %<tag>s : le pipeline de %<mr_url>s a echoue et l'evaluation automatique n'a pas abouti. Intervention manuelle requise.",
      pipeline_non_code: ":warning: %<tag>s : le pipeline de %<mr_url>s echoue pour une raison hors code. Intervention manuelle requise.\n\n> %<explanation>s",
      pipeline_max_rounds: ":warning: %<tag>s : le pipeline de %<mr_url>s echoue a cause du code mais le nombre maximum de rounds de fix est atteint. Intervention manuelle requise.\n\n> %<explanation>s",
      pipeline_fix_error: ':x: %<tag>s : echec de la correction du pipeline — %<error>s',
      pipeline_fix_success: ':wrench: %<tag>s : correction du pipeline appliquee sur %<mr_url>s — %<count>s job(s) corrige(s) (round %<round>s)'
    },
    en: {
      ...
      pipeline_no_failed_jobs: ':warning: %<tag>s: pipeline for %<mr_url>s failed but no failed jobs were found. Manual intervention required.',
      pipeline_infra_pretriage: ":warning: %<tag>s: pipeline for %<mr_url>s failed due to infrastructure (pre-triage). Manual intervention required.\n\n> %<explanation>s",
      pipeline_eval_failed: ':warning: %<tag>s: pipeline for %<mr_url>s failed and automatic evaluation could not determine the cause. Manual intervention required.',
      pipeline_non_code: ":warning: %<tag>s: pipeline for %<mr_url>s failed for a non-code reason. Manual intervention required.\n\n> %<explanation>s",
      pipeline_max_rounds: ":warning: %<tag>s: pipeline for %<mr_url>s failed due to code but the maximum number of fix rounds has been reached. Manual intervention required.\n\n> %<explanation>s",
      pipeline_fix_error: ':x: %<tag>s: pipeline fix failed — %<error>s',
      pipeline_fix_success: ':wrench: %<tag>s: pipeline fix applied on %<mr_url>s — %<count>s job(s) fixed (round %<round>s)'
    }
  }.freeze
