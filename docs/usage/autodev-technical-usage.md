...
- Un client **encore présent** tient son thread Puma par conception : ce n'est pas une mort silencieuse à détecter, et aucun des deux correctifs SSE précédents (timeout sur `Queue#pop`, listener `pagehide`) ne s'y applique. `config/puma.rb` déclare donc `force_shutdown_after 2` (Autodev #112). Sans cette option — **absente des défauts de Puma** (puma 6.6.1, `lib/puma/server.rb`) — `graceful_shutdown` joignait le thread pool **sans timeout** ; comme il ferme le socket d'écoute *avant* d'attendre, un process SIGTERM'é libérait le port immédiatement (le suivant pouvait le binder) tout en continuant de tourner en dessous, `~/.autodev/autodev.db` + WAL + shm ouverts en écriture : 40 minutes observées en production le 03/09/2026. La valeur n'est pas la durée totale : `Puma::ThreadPool#shutdown` ajoute ~5,1 s fixes après son expiration (`SHUTDOWN_GRACE_TIME`, non réglable depuis le DSL), soit ~7,1 s mesurées — à tenir sous les 10 s de `Autodev::Supervisor::TERM_GRACE_SECONDS`, qui est la contrainte liante (l'`ExitTimeOut` de launchd, 20 s par défaut, est plus large). Contrepartie assumée : une requête synchrone lente peut être coupée — `IssuesController#reset` (`Autodev::ResetReclaim`, deux écritures GitLab in-request) et `DeployReviewsController#index` peuvent dépasser 2 s sur un lien lent. Prouvé par `test/sse_shutdown_test.rb` : un vrai `puma -C config/puma.rb` dans un process séparé, un vrai socket laissé ouvert sur `/stream`, un vrai `SIGTERM`.
...
- **danger-claude** — l'outil peut-il tourner du tout ? Aucune sonde nouvelle : `Autodev::UsageGate.probe!` appelait déjà `danger-claude` une fois par cycle pour le quota, et cet appel prouve toute la chaîne (binaire sur le PATH, runtime de conteneur joignable, volume monté, identifiants acceptés, quota disponible) — mais `UsageChecker#available?` écrasait tout dans un booléen. Il répond désormais un verdict typé (`available`, `quota_exhausted`, `auth_refused`, `binary_missing`, `broken`, `unknown`) et cette carte en garde les causes (Autodev #108). **`Hors service` (503)**, contrairement à toutes les autres cartes secondaires : une revue cassée dégrade une passe, un `danger-claude` qui ne démarre pas arrête l'implémentation, la revue *et* la correction des discussions — chaque job échoue. `quota_exhausted` reste `OK` ici (il a son alarme sur *Quota Claude*) et `unknown` aussi (ne pas réussir à observer n'est pas une panne, Autodev #62). `broken` — le fourre-tout des sorties non nulles ne portant ni la signature du quota ni celle de l'authentification — est **débouncé** : `Attention` au premier verdict, `Hors service` au second consécutif, un « consécutif » comptant les sondes *non saines* d'affilée sans trou plus large que le TTL de la sonde. Un hoquet de conteneur ne réveille donc personne, tandis que la panne Docker de neuf heures du 02-03/09, qui a produit 262 verdicts d'affilée, aurait été signalée au second.
- **Requêtes GitLab** — combien d'appels autodev adresse à GitLab, et combien de coupures de transport il a rencontrées (Autodev #96). Passive : le compteur est alimenté par un proxy autour du client GitLab lui-même — un seul endroit dans tout le produit le construit — donc lectures **et** écritures sont comptées sans qu'aucun des quatorze sites d'appel change, pagination incluse. La carte ne lève rien par elle-même : elle existe pour donner un chiffre mesuré là où l'instruction du ticket n'avait qu'une estimation dérivée du code (~30 à 45 requêtes par cycle de deux minutes, dominées par les jobs de surveillance de pipeline), et pour que la corrélation horaire des coupures se lise sur plusieurs jours au lieu de quatre sondages à la main.
...
| `bin/rails autodev:recheck_review_arrears` | Renvoie vers la vérification de pipeline les demandes abandonnées sur un budget de review épuisé **sans avoir jamais été relues** (`review_count` à 0). `LIMIT=N` borne le lot par exécution (défaut 3 = `max_workers`, plafond 10 ; une valeur hors bornes ou non entière fait échouer la tâche avant tout examen), `INCLUDE_AUTHOR_HANDBACK=1` élargit le filtre de propriété aux tickets qu'autodev a lui-même rendus à leur auteur, `INCLUDE_HUMAN_HELD=1` à un ticket détenu par n'importe quelle personne (Autodev #98 : 4 des 20 lignes sont détenues par quelqu'un d'autre que l'auteur). Ce que les trois niveaux gardent en commun est la seule protection qui compte : personne n'a touché la ligne depuis l'abandon — aucun commentaire humain sur le ticket, **aucune note humaine sur la merge request**, aucun label de workflow déplacé. GitLab Community ne tient qu'un assignee, donc le ré-armement **prend** le ticket au lieu de s'y ajouter : il le dit en commentaire en nommant la personne, enregistre qui dans `issues.displaced_assignee_id`, relit l'écriture (Community l'accepte et l'ignore en silence) et le lui rend à la fin plutôt qu'à l'auteur. Une MR **conflictuée** n'est pas éligible (Autodev #105) : `mr_verdict` lit les deux champs que le rapport imprime déjà trois lignes plus bas (`has_conflicts`, `detailed_merge_status`) et décline sous le verdict `:mr_conflicted` — re-armer une MR qui ne peut pas atterrir prenait le ticket à celui qui le tenait pour lancer une boucle de correction à vide (deux des six lignes éligibles du 02/09/2026 étaient dans ce cas, ~34 h et ~39 h de modèle dépensées avant abandon sur stagnation). Une lecture qui **ne répond pas** — `detailed_merge_status: "checking"`, ou `has_conflicts` à `nil` — vaut `:waiting`, jamais `:eligible` : la row reste dans l'arriéré et la prochaine exécution repose la question. Le verdict est compté dans `VERDICTS` et rapporté `conflicted N` à côté de `mr closed`, parce qu'une MR conflictuée reste dès lors dans l'arriéré indéfiniment : la décision suivante (passe de rebase, balayage humain, clôture en masse) se prend sur un nombre plutôt que sur rien. Arriéré d'Autodev #88. |
...
| `AUTODEV_BOOT_GUARD_STRICT` | `1` fait **refuser** le démarrage dans deux cas : un processus qu'autodev ne peut pas identifier comme un de ses propres orphelins retient le fichier de base (Autodev #92), et — depuis Autodev #109 — un orphelin **reconnu** qui est toujours `:alive` après l'escalade. Il ne refuse délibérément **pas** après un reap réussi (`:gone_on_term` / `:gone_on_kill`) : refuser là laisserait un opérateur en cours d'investigation manuelle incapable de démarrer du tout. Par défaut le garde-fou se contente d'un avertissement et le boot continue : le processus qui démarre retient lui-même la base à ce moment-là, et un refus est une panne totale rejouée à chaque redémarrage de launchd. À armer le temps d'une investigation à la main. |

`BootGuard#reap` **escalade** TERM → KILL après une grâce de 10 s via `Autodev::ProcessStopper.stop` (`lib/autodev/process_stopper.rb`), qui renvoie l'un de trois résultats **observés** — `:gone_on_term`, `:gone_on_kill`, `:alive` — et logue la découverte puis le résultat en **deux phrases séparées** (quatre clés de locale `cli_boot_guard_orphan_*` en `fr` et `en`, là où une seule phrase se terminait par « Stopping it (TERM) », c'est-à-dire un résultat annoncé avant d'être connu — Autodev #109 : le puma alpha-52 avait survécu 40 minutes à ce TERM le 03/09/2026, base + WAL + shm ouverts en écriture). Un boot avec deux enfants abandonnés peut donc passer jusqu'à 24 s à les reaper (grâce + confirmation du KILL, par orphelin, séquentiellement). `Supervisor#shutdown_children` n'adopte **pas** ce module, pour deux raisons écrites au-dessus de la méthode : `ProcessStopper.alive?` est `Process.kill(0, pid)`, qui ne distingue pas un process vivant d'un zombie non reapé (les enfants du superviseur, eux, sont réellement les siens à reaper), et `ProcessStopper` est par-pid alors que `wait_for_graceful_exit` attend tous les enfants sur une **seule** échéance partagée — l'adopter ferait attendre deux traînards l'un après l'autre (~20,7 s mesurées contre ~10 s aujourd'hui), au-delà des 20 s d'`ExitTimeOut` par défaut de launchd. Deux escalades exprès, pas par accident.
...
7. `dispatch_retries` — issues `error` + `pending` avec backoff écoulé → `:retry_errored` / `:retry_stuck`. Les **deux** chemins effacent `next_retry_at` (Autodev #111 : `perform_retry_errored` laissait le stamp derrière lui, et `PollDispatcher.retryable?` ne lit que ce stamp + `retry_count` — un résidu rend la row candidate dès que son statut repasse par `error`/`pending`, sans qu'aucun `mark_failed` l'ait décidé ; seul le filtre de statut de `fetch_retryable` protégeait, ce qui est une seconde ligne et pas la règle). `:retry_errored` **relit le ticket avant toute autre chose** et pose la question de la reprise humaine (`ExternalState#stop_on_handover`, atteint via le porteur `Autodev::HandoverStop`) : `dispatch_unassignment` ne balaie que `ACTIVE_STATUSES`, dont `error` ne fait pas partie, alors que c'est l'état où une reprise est la plus probable — le retry reposait `label_doing` sur un ticket que quelqu'un tenait déjà, la reprise n'étant vue qu'un cycle plus tard (Autodev #102). Sur un oui : pas de transition, pas de label restauré, la row est close avec le commentaire de handover existant. Une lecture GitLab qui échoue **décline** le retry pour ce cycle et ne touche à rien — la row reste `error` et sera re-enfilée au cycle suivant, sans budget dépensé (`retry_count` est écrit par `mark_failed`, pas par cette passe). `ACTIVE_STATUSES` n'est délibérément **pas** élargi : c'est une frontière documentée qui gouverne la population d'autres passes
8. `dispatch_infra_recheck` — issues `done` + `needs_attention` + `attention_reason: 'stagnation_pipeline'`, MR ouverte, sous le cap et backoff écoulé → action `:recheck_infra` (re-tentative auto d'une stagnation infra, voir plus bas). Avant toute écriture, la reprise **demande si quelqu'un a touché la demande depuis l'abandon** (`Autodev::UntouchedSinceGiveup`, extrait de `ReviewArrearsSweep` pour que les deux passes posent la même question : aucun commentaire humain sur le ticket, aucun sur la merge request, aucun déplacement de label de workflow). Sans cette porte, un humain qui avait repris le ticket abandonné et réparé la CI se faisait retirer l'assignation — GitLab Community n'accepte qu'un assigné — par un commentaire lui annonçant que son infrastructure s'était rétablie. Une lecture GitLab qui échoue **décline** la reprise pour ce cycle : un ticket illisible n'est jamais une permission de le prendre. La passe **réserve la row avant d'enfiler** le job (Autodev #110) : un `UPDATE` conditionnel qui rejoue le prédicat en course (statut, flag, raison, compteur, backoff — pas `mr_iid`, que rien ne met en course) et re-stampe `infra_recheck_at`. C'est un compare-and-set : deux cycles en course sur la même row ne peuvent pas matcher tous les deux. Sans ça, les deux colonnes que la passe **sélectionne** n'étaient écrites que par le job : mesuré le 04/09/2026 sur powerpanne/core#16030, les quatre premières tentatives se sont espacées d'une heure comme le backoff l'entend, puis cinq enfilements en quatre-vingts secondes ont tous annoncé « tentative 5 » et les jobs ont tourné `5/5` → `9/5` — un budget de cinq regards horaires sur une CI qui récupère dépensé en quatre minutes, et la row devenue sélectionnable par aucune passe automatique
...
**Deux chemins pour une même étape (Autodev #74).** `launch_review` bifurque sur `@project_config['review_skill']` : absent → le binaire `mr-review`, comme avant ; renseigné → `SkillReviewer#review_with_skill` clone la branche de la MR dans un répertoire de travail dédié, y injecte les skills du projet et lance le skill déclaré via `danger-claude -p`, en lui interdisant d'écrire sur GitLab — il dépose son verdict dans un fichier de contrat (`ReviewContract`) que `ReviewPublisher` seul traduit en discussions/commentaire. Les deux chemins répondent par une **issue nommée**, tranchée par `dispatch_review_outcome` (Autodev #107 a remplacé l'ancien axe `true`/`false`/`:inconclusive` : `false` n'a jamais été un verdict sur la merge request, c'étaient trois causes sans rapport portant un même booléen). `true` → `finalize_review_success`. `:unusable_output` (chemin skill — le fichier de contrat est absent ou hors schéma) → `finalize_review_failure`, budget de 5 échecs consécutifs : c'est la seule cause qui parle de *cette* demande rencontrant *ce* skill, donc la seule où un compteur par demande est le bon instrument. `:unanchored` → abandon sous `review_findings_unanchored`. `:tool_missing` (chemin binaire — `mr-review` n'est pas sur le PATH) → abandon immédiat sous `review_tool_missing` : c'est un fait de configuration, vrai à chaque poll jusqu'à ce que quelqu'un installe l'outil, donc l'attendre n'a pas de sens. Et `:inconclusive` (GitLab n'avait pas encore calculé les `diff_refs`), `:tool_unavailable` (l'outil n'a pas pu tourner : runtime de conteneur, timeout, crash, code de sortie non nul) et `:clone_failed` (le clone de la revue n'a pas abouti) → retour direct à `checking_pipeline` via `review_done!`, **sans toucher aucun des deux compteurs** et **sans réestampiller l'horloge de surveillance**, pour que le poll suivant refasse la revue en entier sans que la borne d'âge ait été repoussée. Ces trois-là n'écrivent **aucune ligne d'activité par poll** — une ligne par cycle sortirait la demande des trois bras de `DormantAudit` pendant toute la panne — et n'ont **pas de compteur propre** : un compteur par cause a été essayé puis retiré, inerte sur un message qui porte le temps de connexion de git et gâchette sur un message qui ne le porte pas. La borne des trois est la borne d'âge, `pipeline_watch_max_days`, dont la phrase d'abandon — « je surveille depuis plus de N jours sans jamais pouvoir conclure » — est vraie de chacune.
...
**La garde de stagnation n'était atteignable que depuis les tours qui convergeaient (Autodev #99).** `run_fix_cycle` se termine de deux façons — `finalize_no_commits` quand le tour n'a produit aucun commit, ou pousser puis `finalize_success` — et `discussion_stagnated?`, qui **compte** l'occurrence *et* **prend** la décision, ne vivait que dans la seconde. Un tour sans commit, qui est exactement ce à quoi ressemble une stagnation vue de l'intérieur, sortait avant la garde et ne faisait ni l'un ni l'autre. Mesuré sur powerpanne 15205 le 02/09/2026 : dix-huit tours en seize heures, chaque discussion répondant `discussion_unchanged`, le quota Claude épuisé **deux fois en douze heures**, et `stagnation_signatures` lisant `count: 1` au tour 17. La forme du jumeau pipeline est la correction : la garde est inconditionnelle, le comptage est conditionné à une tentative aboutie (règle d'Autodev #71, intacte — ce tour EST une tentative aboutie : les fils ont été lus, les corrections tentées, danger-claude a répondu).

**Et un plafond au-dessus des deux**, parce que « la garde est inatteignable » est désormais une chose qui est arrivée : `MrFixer#fix_round_ceiling` = `stagnation_threshold * 3`, contrôlé en tête de `run_fix_round` avant la liste des fils, avant tout clone, avant tout appel de modèle — une ligne au-delà ne dépense rien. Ce n'est délibérément pas une seconde garde de même nature : il lit un entier sur la ligne et le compare, donc aucune évolution de la signature ni de la structure des branches ne peut le rendre inerte. L'entier est `issues.discussion_fix_round`, sa **propre colonne** : `fix_round` est avancé par les deux boucles — celle des discussions et celle des jobs en échec — et lire ce compteur partagé abandonnait des demandes saines (treize tours de pipeline sur des jobs différents, donc jamais de stagnation pipeline, puis deux tours de discussion, et le plafond se déclenchait en accusant une garde qui avait compté deux fois). `fix_round` garde son sens et ses trois lecteurs (log, notification GitLab, dashboard), qui parlent du travail fait sur la demande, c'est-à-dire de la somme des deux boucles. Motif d'abandon propre, `fix_rounds_exhausted`, jamais `stagnation_discussions` : y arriver signifie que la garde ordinaire n'a pas fait son travail, et dire « stagnation » masquerait précisément le fait qui mérite d'être vu. `dispatch_infra_recheck` ne vise que `stagnation_pipeline`, donc ce motif ne ré-arme rien.
...
**Qui écrit quoi** (Autodev #110) : la passe `dispatch_infra_recheck` possède `infra_recheck_at` — l'horloge sur laquelle elle sélectionne, donc re-stampée par sa réservation avant l'enfilement — et `InfraRecheck#record_recheck_attempt` possède `infra_recheck_count`, écrit **uniquement** quand le verdict est `:spend`. Le compteur reste volontairement dans le job : une panne GitLab ou un état de MR transitoire ne lisent rien et ne doivent donc pas brûler le budget de surveillance ; le déplacer à l'enfilement réintroduirait le même dégât par l'autre bout. Le cap est aussi une garde à l'écriture : une tentative au-delà de `infra_recheck_max` est **refusée et loguée** au lieu d'être enregistrée (c'est ainsi qu'un `9/5` a pu rester en base avant #110).
...
| `mr-review` non installé | Warning au démarrage **si au moins un projet ne déclare pas de `review_skill`** (Autodev #74). Une demande qui atteint l'étape de revue sur un tel projet répond `:tool_missing` et est **abandonnée aussitôt** sous `review_tool_missing` : `abandon` → `done` + `needs_attention` + `label_attention` + ticket rendu + commentaire nommant l'outil manquant. C'est un fait de configuration, pas une panne qui passe, et c'est la réponse que `MissingReviewSkillError` obtient déjà sur l'autre chemin. Défensif aujourd'hui : les deux projets configurés déclarent un `review_skill`, donc le chemin binaire n'est pas pris. Un projet sur le chemin skill lance sa revue via `danger-claude`, qui est une dépendance dure : rien n'est skippé pour lui |
...
| GitLab refuse la même requête à chaque cycle (400 ou 422) | `InvalidRequestError` (Autodev #95), sous-classe d'`ApiUnavailableError` comme `MissingTargetBranchError` — le parent encode « cette unité de travail ne peut pas conclure », ce qui est le cas, et toutes les frontières la traitent déjà. Ce que la famille ne transmet PAS, c'est l'attente : un 5xx ou un timeout justifient de réessayer, un 400 non. GitLab a répondu, et il dit que la requête est invalide ; la rejouer à l'identique ne peut pas marcher. `InvalidRequestBound` compte les occurrences de la **même** cause — endpoint, statut et plainte de GitLab — puis abandonne sous `gitlab_refused_request`, motif qui nomme la vraie raison. Observé en production le 01/09/2026 : le POST des discussions a répondu `400 line_code must be a valid line code`, parce que les positions ne se résolvaient pas sur le diff que GitLab détenait alors. **La cause d'abord écrite ici était fausse** : on avait retenu le conflit de la merge request, et on en avait conclu que la condition était permanente. Elle ne l'était pas — la même merge request, toujours en conflit, a publié dix-sept discussions ancrées quatre heures plus tard, une fois ses références de diff calculées. Ce que le refus ne dit pas, c'est s'il est transitoire ou définitif, et c'est précisément pourquoi la borne est le bon remède : elle protège des deux sans avoir à trancher ; la revue réussissait — dix-huit minutes de modèle, constats produits — et la publication échouait, cinq tours de suite, à raison de dix-huit minutes toutes les vingt. Le verdict `:inconclusive`, lui, n'a pas besoin de borne propre : il termine le poll normalement, donc `abandon_expired_watch` est atteint et l'horloge lue est la vraie. C'est exactement ce que l'exception sautait. `ReviewPublisher` retombe d'abord sur un commentaire non ancré : une revue qui a produit ses constats ne les perd pas parce qu'elle ne peut pas les ancrer. Le compte affiché est celui des refus portant **la même réponse**, pas des polls consécutifs : un cycle sain entre deux refus ne compte ni n'efface, et les trois sinks le disent ainsi. Le commentaire cite la réponse de GitLab mot pour mot et ne suppose aucune cause : rien sur ce chemin ne lit `has_conflicts` / `merge_status` (revue neutre du #95). Le coût du remède est chiffré comme celui du défaut : cinq polls avant l'abandon, chacun repayant une revue complète, environ quatre-vingt-dix minutes de modèle — une fois, contre une boucle sans fin |
| Revue en `changes_requested` dont aucun constat n'a pu être ancré | Pas une livraison (Autodev #95, revue neutre). `green_post_review` décide sur les fils de discussion non résolus et sur rien d'autre, et un commentaire de synthèse est une note, pas un fil résoluble : une revue dont tous les constats sont repliés laissait donc le poll suivant lire une merge request propre et relue, et la demande finissait `done`, non signalée, sous `label_done`. Mesuré : `STATUS=done attention=false reason=nil label_done=true`. `SkillReviewer#unanchored_verdict?` pèse maintenant `contract.verdict` contre le nombre de constats réellement ancrés, et `abandon` → `done` + `needs_attention` (`review_findings_unanchored`) + `label_attention` + ticket rendu à son auteur + commentaire. Ce n'est pas un échec de revue — elle a tourné et jugé — et un verdict `approve` n'est pas concerné : il ne demande rien à personne |
| Branche cible absente du dépôt, prouvée absente | `MissingTargetBranchError` (Autodev #91), sous-classe d'`ApiUnavailableError` parce qu'elle sort par les mêmes frontières — mais elle porte un drapeau `confirmed?`, et c'est tout le sujet. `fetch_target_with_history` ignore son propre statut, donc une panne réseau et une branche supprimée lèvent la même exception : borner la classe entière aurait rendu un ticket sain à son auteur sur un hoquet de VPN. `ensure_base_available!` demande donc au remote (`git ls-remote --heads`, une requête, et seulement sur le chemin qui va lever de toute façon) et distingue trois cas — le remote répond qu'il n'a pas la branche (**preuve**), le remote l'a mais le fetch n'a pas abouti (panne), le remote ne répond pas (panne). Le défaut est `confirmed: false`, pour qu'un nouvel appelant doive énoncer sa preuve. **Sur preuve seulement**, `MissingBaseBound` compte `stagnation_threshold` occurrences de la *même* branche puis `abandon` → `done` + `needs_attention` (`target_branch_missing`) + `label_attention` + ticket rendu à son auteur + commentaire nommant la branche et le nombre de tentatives. Branché aux **deux** frontières (`PipelineMonitor#check` et `MrFixer#fix`), parce que `dispatch_pipelines` et `dispatch_discussions` réenfilent chacune toute leur population à chaque cycle. Sans cette borne, la ligne clonait toutes les 120 s indéfiniment sans aucun signal : la frontière se contentait de journaliser, la borne d'âge était sautée, la signature de stagnation n'était pas écrite, et la ligne d'activité collapsée avançait sa date, ce qui la rendait invisible de `DormantAudit`. La documentation appelait ça « la ligne attend » ; c'était une fuite (revue neutre du 01/09/2026) |
...
| Boucle de correction passant `stagnation_threshold * 3` tours quoi que dise sa signature | `done` + `needs_attention` (`fix_rounds_exhausted`) + `label_attention` + ticket rendu + commentaire (Autodev #99). Un plafond, pas une garde : lu sur `issues.discussion_fix_round` avant la liste des fils et avant tout clone, donc une ligne au-delà ne dépense rien. Ce contre quoi il protège n'est pas une stagnation — la garde au-dessus s'en charge — mais le fait que cette garde soit inatteignable, ce qui est arrivé. Motif propre à dessein |
...
| Un humain clôture / réinitialise / force une transition pendant qu'un travail long tourne | La transition terminale du job est **refusée** (Autodev #97). `persist_status_change!` compare la ligne à `aasm.from_state` et lève `StaleTransitionError` ; les cinq frontières qui peuvent l'atteindre — `PipelineMonitor#check`, `MrFixer#fix`, `IssueProcessor#process`, plus les deux que le poll décide en ligne, `PollDispatcher#check_external_state` et `PollRouter#route` — y répondent en loguant et en n'écrivant rien du tout, ce qui compte parce que deux d'entre elles marqueraient sinon la ligne en échec, stockeraient un message et posteraient un commentaire GitLab sur un ticket que quelqu'un vient de reprendre. La frontière est **par ligne** : les deux du poll ne rattrapaient qu'une erreur chacune, donc un refus y remontait au `rescue StandardError` de `dispatch` et emportait les passes du cycle qui n'avaient pas encore tourné, pour tout le projet. Observé le 01/09/2026 : une demande fermée depuis le tableau de bord a été ressuscitée huit minutes plus tard par la revue qui tournait encore, et a continué vingt minutes. Ce que devient le travail déjà fait est décidé et non subi : ce qui a atteint la branche y reste — c'est sur GitLab et le poll suivant le relirait tel quel — et ce qui est abandonné est la prétention, le changement d'état et le commentaire qui l'annonce |
| Relance d'une demande en `error` | `retry_pipeline!` → `checking_pipeline` ou `retry_processing!` → `pending`, et le label posé est `label_doing` dans les deux cas (Autodev #100). Avant, la relance bifurquait sur « la demande porte-t-elle une MR » et posait `label_done` quand oui — or les deux destinations sont des états de **travail**, donc elle annonçait le travail comme terminé pendant qu'autodev continuait dessous. Mesuré sur powerpanne 15205 : ticket posé dans la colonne de relecture du PM à 00:50 et resté dix heures pendant que les tours 5 à 18 tournaient. La bifurcation était juste quand elle a été écrite, contre une config à cinq labels où `label_mr` signifiait « une MR existe » ; ce label a ensuite été *renommé* `label_done`, reprenant le sens d'un autre, et le site d'appel a gardé son nom de méthode |
...
| Label de workflow déplacé vers une autre valeur du scope d'autodev | Idem. Le scope est déduit de `label_doing` + `label_done` (`label_attention`, qu'Autodev écrit lui aussi, est retiré de l'ensemble des labels « libres » mais reste hors de la dérivation) ; les labels hors scope (`PM::Evolution`, `Backlog`, noms de clients…) sont ignorés, et la règle se désactive d'elle-même si ces deux labels ne partagent aucun scope. L'auteur de l'édition est lu dans les *resource label events* GitLab, et seulement quand la lecture gratuite des labels a déjà un candidat — un ticket sain ne coûte aucun appel API supplémentaire. La valeur étrangère **est** la preuve, et une écriture d'autodev qui la retire efface la réponse : le nettoyage du scope est donc **opt-in** (Autodev #98), avec un seul appelant — `repose_working_label`, atteint depuis le seul rattrapage qui établit que personne n'a touché la ligne avant d'écrire — et confiné aux labels qu'autodev **possède**, les deux projets configurés déclarant un label d'entrée scopé : un test sur le scope effacerait la colonne d'un relecteur pendant qu'une demande attend ce même relecteur |
