...
| Projet déclarant un `review_skill` absent du clone (`.claude/skills/<nom>/SKILL.md`) | `ConfigError` (Autodev #74), volontairement hors du rescue de `review_with_skill` **et** des trois bras de `launch_review` : ce n'est pas un échec de review, aucun compteur ne bouge, et seul le `rescue StandardError` de `PipelineMonitor#check` l'attrape pour la logger. La ligne reste en `reviewing`, qu'aucune passe ne re-sélectionne — c'est `DormantAudit` qui finit par la signaler (`dormant_exhausted`). L'attraper aurait écrit un `ActivityEvent` à chaque poll, donc une boucle non bornée et non signalée : se garer est le moins mauvais des deux comportements disponibles |
