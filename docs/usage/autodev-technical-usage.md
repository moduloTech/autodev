...
| `AUTODEV_BOOT_GUARD_STRICT` | `1` fait **refuser** le démarrage quand un processus qu'autodev ne peut pas identifier comme un de ses propres orphelins retient le fichier de base (Autodev #92). Par défaut le garde-fou se contente d'un avertissement et le boot continue : le processus qui démarre retient lui-même la base à ce moment-là, et un refus est une panne totale rejouée à chaque redémarrage de launchd. À armer le temps d'une investigation à la main. |
